...
class StoreAValue(webapp2.RequestHandler):
  ...
  def store_a_value(self, tag, value):
	#test = json.loads("{\"op\": \"submit\", \"data1\": \"vehicleId\", \"data2\": \"gas amount\"}")
	#test = json.loads("{\"op\": \"confirm\", \"data1\": \"vehicleId\", \"data2\": \"\"}")  return if the transaction was aproved or not this on the GET VALUE part
...
class GetValueHandler(webapp2.RequestHandler):
  ...
  def get_value(self, tag):
	#test = json.loads("{\"op\": \"login\", \"data1\": \"user1\", \"data2\": \"password\"}") <--all this as a tag
	#config = json.loads(value)
...
def WriteJson(handler, tag,value):
