...
from userm import Fields
from userm import Userm
from abstract import AbstactDAO

class Users(webapp2.RequestHandler, AbstactDAO):
	...
	def create(self):
		...
		if params['name'] and params['user'] and params['password']:
			params['id'] = self.getLastKey()+1
			entry = db.GqlQuery("SELECT * FROM Userm where id = :1", params['id']).get()
			if entry:
				json.dump({"result": False, "message":self.msgHash['DUPLICATE_KEY']}, self.response.out)
				return
			else:
				entry = self.loadModelObj(Userm(), params)
			entry.put()
	...
	def update(self):
		params = self.getRequestData()
		if params['id']:
			entry = db.GqlQuery("SELECT * FROM Userm where id = :1", self.convertToInteger(params['id'])).get()
			if entry:
	...
	def delete(self):
		...
		if id:
			entry = db.GqlQuery("SELECT * FROM Userm where id = :1", id).get()
			if entry:
				db.delete(entry)
