...
from driver import Fields
from abstract import AbstactDAO

class Drivers(webapp2.RequestHandler, AbstactDAO):

	mappings = Fields.mappings

	def list(self):
		entries = db.GqlQuery("SELECT * FROM Driver ORDER BY id asc")
		items = []
	...
	def create(self):
		...
		if params['name']:
			...
			entry = db.GqlQuery("SELECT * FROM Driver where id = :1", params['id']).get()
			if entry:
				json.dump({"result": False, "message":self.msgHash['DUPLICATE_KEY']}, self.response.out)
				return
			else:
				entry = self.loadModelObj(Driver(), params)
			entry.put()
	...
	def update(self):
		params = self.getRequestData()
		if params['id']:
			entry = db.GqlQuery("SELECT * FROM Driver where id = :1", self.convertToInteger(params['id'])).get()
			if entry:
	...
	def delete(self):
		...
		if id:
			entry = db.GqlQuery("SELECT * FROM Driver where id = :1", id).get()
			if entry:
				db.delete(entry)
